/**
 * The Main class is the entry point of the program. It demonstrates the usage of various shape classes
 * and their methods for calculating area, cloning, scaling, and comparing.
 */
public class Main {
    public static void main(String[] args) {

        Square square = new Square(5);
        Square square3 = new Square(2);

        Deltoid deltoid = new Deltoid(3, 4, 60, 120, 5, 6);
        Deltoid deltoid2 = new Deltoid(3, 4, 60, 120, 7, 9);

        Irregular irregular = new Irregular(3, 4, 5,2,20,60,70, 80, 90, 100);
        Irregular irregular2 = new Irregular(3, 4, 5,2,20,60,70, 80, 90, 100);
        
        IsoscelesTrapezium isoscelesTrapezium = new IsoscelesTrapezium(3, 4, 5, 60, 30, 6);
        IsoscelesTrapezium isoscelesTrapezium2 = new IsoscelesTrapezium(3, 4, 5, 60, 30, 6);
        
        Parallelogram parallelogram = new Parallelogram(3, 4, 60, 120, 5,6);
        Parallelogram parallelogram2 = new Parallelogram(3, 4, 60, 120, 5,6);

        Rectangle rectangle = new Rectangle(3, 4);
        Rectangle rectangle2 = new Rectangle(3,4);

        Rhombus rhombus = new Rhombus(3, 60, 30, 6, 5);
        Rhombus rhombus2 = new Rhombus(3, 60, 30, 6, 5);

        Trapezium trapezium = new Trapezium(3, 4, 5, 6, 60, 60, 120, 120, 7, 8);
        Trapezium trapezium2 = new Trapezium(3, 4, 5, 6, 60, 60, 120, 120, 7, 8);

        System.out.println("Square Area: " + square.calculateArea());
        System.out.println("Deltoid Area: " + deltoid.calculateArea());
        System.out.println("Deltoid2 Area: " + deltoid2.calculateArea());
        System.out.println("Irregular Area: " + irregular.calculateArea());
        System.out.println("IsoscelesTrapezium Area: " + isoscelesTrapezium.calculateArea());
        System.out.println("Parallelogram Area: " + parallelogram.calculateArea());
        System.out.println("Rectangle Area: " + rectangle.calculateArea());
        System.out.println("Rhombus Area: " + rhombus.calculateArea());
        System.out.println("Trapezium Area: " + trapezium.calculateArea());

        System.out.println(square);
        System.out.println(deltoid);
        System.out.println(irregular);
        System.out.println(isoscelesTrapezium);
        System.out.println(parallelogram);
        System.out.println(rectangle);
        System.out.println(rhombus);
        System.out.println(trapezium);
        
        Square square2 = square.clone();

        System.out.println(square.clone().equals(square));

        System.out.println(square2);  
        
        square.scale(2);

        System.out.println(square);

        System.out.println(square2);

        System.out.println(square.equals(square2));

        System.out.println(square.compareTo(square3));
        System.out.println(deltoid.compareTo(deltoid2));
        System.out.println(irregular.compareTo(irregular2));
        System.out.println(isoscelesTrapezium.compareTo(isoscelesTrapezium2));
        System.out.println(parallelogram.compareTo(parallelogram2));
        System.out.println(rectangle.compareTo(rectangle2));
        System.out.println(rhombus.compareTo(rhombus2));
        System.out.println(trapezium.compareTo(trapezium2));
    }
}
